...
namespace Transport
{
    ...
    public class TestBase
    {
        ...
        protected const uint SMALL_FILE = 1024 * 1024 * 10; //10MB
        protected const uint BIG_FILE = 1024 * 1024 * 100; //100MB
        ...
        public void Init()
        {
            ...
            var manager = new ModelManager(drawingPath);

            _server = new TcpMetalServer();
            _client = new TcpMetalClient();

            _server.AddDatagramHandler(new GetElementHandler(manager));
            _server.AddDatagramHandler(new GetElementsListHandler(manager));
            _server.AddDatagramHandler(new RemoveElementHandler(manager));
            _server.AddDatagramHandler(new SecurityHandler(manager));
            _server.AddDatagramHandler(new SetElementHandler(manager));
            _server.AddDatagramHandler(new GetPlanHandler(manager));
        ...
        private void ClearAll()
        {
            ...
            _dataManager.ExcuteRequestAsync<SetListData>(request, CancellationToken.None).
                   ContinueWith((Action<Task<SetListData>>)((t1) =>
                   {
                       ...
                       {
                            //материалы лимитки
                            request = factory.WithType((DatagramType)DatagramType.GetLimitMaterialList).
                            WithDTOObject<GetListData>((GetListData)new GetListData((long)0, (long)int.MaxValue, (bool)false)).
        ...
        protected EmployeeDTO GenerateAndStoreEmployee(Guid postId, Guid gorupId)
        {
            var employee = GenerateEmployee(postId, gorupId);
        ...
        protected OrderDTO GenerateAndStoreOrder(Guid customerId, Guid? fileId, Guid orderGroupId)
        {
            var order = GenerateOrder(customerId, fileId, orderGroupId);
        ...
        {
            var factory = new DatagramFactory();
            var request = factory.WithType(DatagramType.SetOrderElement).WithDTOObject(order).Build();
            ...
            var task = requestManager.ExcuteRequestAsync<HandledDTO>(request, CancellationToken.None).
                ContinueWith((t1) => Assert.IsTrue(CheckError(t1)));
        ...
        {
            var factory = new DatagramFactory();
            var request = factory.WithType(DatagramType.RemOrderElement).WithDTOObject(new RemElementData(id, permannet)).Build();
            _dataManager.ExcuteRequestAsync<HandledDTO>(request, CancellationToken.None).
                ContinueWith((t1) =>
                {
                    Assert.IsTrue(CheckError(t1));
        ...
        protected OrderDTO GenerateOrder(Guid customerId, Guid? fileId, Guid orderGroupId)
        {
            ...
            order.SendDeliveryDate = DateTimeHelper.DateTimeNow();
        ...
        protected EmployeeDTO GenerateEmployee(Guid postId, Guid groupId)
        {
