        ...
        : ViewModelBase
    {
        ...
        protected CommandViewModel Uninstall()
        {
            return new CommandViewModel
            {
                ...
                Command = new RelayCommand(() =>
                {
                    if (OnNext())
                    {
                        if (MessageBox.Show($"Вы действительно хотите удалить Metal server?", $"Удаление Metal client",
        ...
        protected CommandViewModel Cancel()
        {
            return new CommandViewModel()
            {
                ...
                Command = new RelayCommand(() =>
                {
                    if (MessageBox.Show($"Вы действительно хотите отменить установку Metal server?", $"Установка Metal server",
                        MessageBoxButton.OKCancel, MessageBoxImage.Question, MessageBoxResult.OK) == MessageBoxResult.OK)
                    {
                        if (OnCancel())
                        {
        ...
        protected CommandViewModel CancelUninstall()
        {
            return new CommandViewModel()
            {
                ...
                Command = new RelayCommand(() =>
                {
                    if (MessageBox.Show($"Вы действительно хотите отменить удаление Metal server?", $"Удаление Metal server",
                        MessageBoxButton.OKCancel, MessageBoxImage.Question, MessageBoxResult.OK) == MessageBoxResult.OK)
                    {
                        if (OnCancel())
                        {
        ...
        protected virtual IEnumerable<CommandViewModel> GetCommands()
        {
            yield return Prev();
            yield return Next();
            yield return Cancel();
